...
This is the org-level `.github` repository for `greenpill-dev-guild`. It has two jobs:
...
- **Workspace migration language is fixed.** References use `greenpill.app/dev-guild` with the qualifier: `coming soon — currently on Charmverse during migration`.
- **Do not conflate the workspace migration with the `charmverse` fork.** They are different things.
...
| Path | Purpose |
| --- | --- |
| [`CONTRIBUTING.md`](./CONTRIBUTING.md) | Guild-wide contributor flow and bounty guidance |
...
- `profile/README.md`, `CONTRIBUTING.md`, and the routines should not drift on bounty flow, payout timing, or workspace language
