...
- Gitcoin Grants + Grant Stack (cross-ecosystem rounds), Optimism RetroPGF + Grants, Arbitrum DAO governance forum, Ethereum Foundation, Protocol Labs / Filecoin Foundation (+ Filecoin devgrants, FIL ProPGF), Octant epochs, Celo Public Goods (Proof of Ship), Stellar Community Fund, Safe grants, EAS grants (+ Astral Protocol — geospatial attestations on EAS, a direct Green Goods infra fit), Giveth / Drips, ENS Public Goods / ENS DAO grants (public goods + naming/identity).
...
- **Cover all three primary targets every run.** The pipeline has historically over-indexed Green Goods while Coop and PGSP stay thin — do not let GG candidates fill the run's quota and crowd out the others. Each run, scan at least one funder for Coop's stack (Filecoin ProPGF / devgrants, Safe grants) and one for PGSP's stack (Lido CSM / LEGO, SSV / DVT, Obol, plus the broader restaking/staking grant surface — EigenLayer ecosystem, Rocket Pool GMC, StakeWise), and actually hit the decentralized-identity surface named in Cluster D (DIF, OpenWallet, Trust-over-IP) — the shared passkey/identity primitive spans GG + Coop and is the most under-scouted lane.
