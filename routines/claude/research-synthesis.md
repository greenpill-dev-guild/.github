...
Defaults: **most runs file nothing.** Title `Research: {short action title}`; team Research; state `Backlog`; labels `activity:research`, `ai:routine`, relevant `protocol:*` (**pass these to `save_issue` as bare child names** — `["research", "routine", "green-goods"]` — since `group:child` is display shorthand the API does not accept, and one unresolvable entry rejects the whole array and files nothing); unprojected (graduate only into an existing bounded project; never into retired/staging projects). Body = the [Brief shape](../../docs/linear-templates.md#brief) with sources, the corpus hook (`Extends: {RESR-x / cycle theme}`), scope, owner (`open` unless someone claimed it), and confidence. No diagrams.
